...
public class CrazyStreams {
    ...
    public Map<String, List<Account>> groupAccountsByEmailDomain() {
        return accounts.stream()
                .collect(Collectors.groupingBy(a -> a.getEmail().substring(a.getEmail().lastIndexOf('@') + 1)));
    }
    ...
    public int getNumOfLettersInFirstAndLastNames() {
        return accounts.stream()
                .mapToInt(a -> (a.getFirstName() + a.getLastName()).length())
    ...
    public BigDecimal calculateTotalBalance() {
        return accounts.stream()
                .reduce(BigDecimal.ZERO, (a, b) -> a.add(b.getBalance()), BigDecimal::add);
    }
    ...
    public boolean containsAccountWithEmailDomain(String emailDomain) {
        return accounts.stream()
                .anyMatch(a -> a.getEmail().substring(a.getEmail().lastIndexOf('@') + 1)
                        .equals(emailDomain));
    }
    ...
    public BigDecimal getBalanceByEmail(String email) {
        return accounts.stream()
                ...
                .findFirst()
                .orElseThrow(() -> new EntityNotFoundException("Cannot find Account by email=%s"
                        .formatted(email)));
    }
